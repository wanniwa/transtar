...
class RequestLimiter:
    """请求速率限制器，用于控制RPM（每分钟请求数）和TPM（每分钟Token数）"""
    ...
    def set_limit(self, tpm_limit: int, rpm_limit: int) -> None:
        ...
        # 设置限制器的RPM参数
        self.request_interval = 60 / rpm_limit if rpm_limit > 0 else 0  # 请求的最小时间间隔（s）
