...
# 简易请求器
class SimpleExecutor(TransBase):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # 订阅接口测试开始事件
        self.subscribe(TransBase.EVENT.API_TEST_START, self.api_test_start)
        # 订阅术语表翻译开始事件
        self.subscribe(TransBase.EVENT.GLOSS_TASK_START, self.glossary_translation_start)
        # 订阅表格翻译任务事件
        self.subscribe(TransBase.EVENT.TABLE_TRANSLATE_START, self.handle_table_translation_start)
        # 订阅表格润色任务事件
        self.subscribe(TransBase.EVENT.TABLE_POLISH_START, self.handle_table_polish_start)
        # 订阅表格派能任务事件
        self.subscribe(TransBase.EVENT.TABLE_FORMAT_START, self.handle_table_format_start)
        # 订阅术语提取任务事件
        self.subscribe(TransBase.EVENT.TERM_EXTRACTION_START, self.handle_term_extraction_start)
        # 订阅术语提取翻译事件
        self.subscribe(TransBase.EVENT.TERM_TRANSLATE_SAVE_START, self.handle_term_translate_save_start)
    ...
    def api_test(self, event, data: dict):
        ...
        # 如果是翻译接口（google/deepl），走专用测试流程
        if (api_format or "").lower() in ("google", "deepl"):
            try:
                self.print("")
                self.info("正在进行接口测试 ...")
                self.info(f"接口名称 - {platform_name}")
                # 针对 Google 翻译：无需密钥
                if (api_format or "").lower() == "google":
                    test_text = "Hello world"
                    res = GoogleTranslateV2().translate(test_text, "zh", "en")
                    if getattr(res, "result", None):
                        self.info(f"接口返回信息 - {res.result}")
                        success.append("google")
                    else:
                        failure.append("google")
                else:
                    # DeepL：支持多密钥用逗号分隔
                    api_keys = re.sub(r"\s+", "", api_key or "").split(",") if api_key else [""]
                    for single_key in api_keys:
                        if not single_key:
                            failure.append("<empty_key>")
                            continue
                        try:
                            translator = deepl.Translator(single_key)
                            res = translator.translate_text("Hello world", source_lang="en", target_lang="zh")
                            if getattr(res, "text", None):
                                self.info(f"接口返回信息 - {res.text}")
                                success.append(single_key[-8:] if len(single_key) > 8 else single_key)
                            else:
                                failure.append(single_key[-8:] if len(single_key) > 8 else single_key)
                        except Exception as e:
                            self.error(f"DeepL 测试失败: {e}")
                            failure.append(single_key[-8:] if len(single_key) > 8 else single_key)
            except Exception as e:
                self.error(f"翻译接口测试异常: {e}")
                failure.append((api_format or "api"))

            # 发送完成事件
            self.event_emit(TransBase.EVENT.API_TEST_DONE, {
                "failure": failure,
                "success": success,
            })
            return
    ...
    #         self.info("没有需要翻译的术语")
    #         self.event_emit(TransBase.EVENT.GLOSS_TASK_DONE, {
    #             "status": "null",
    ...
    #     config.prepare_for_translation(TaskType.TRANSLATION)
    #     platform_config = config.get_platform_configuration("translationReq")
    #     file_source_lang = appConfig.source_language.value
    #
    #     # 翻译任务分割
    #     MAX_LINES = 20  # 最大行数
    ...
    #         check_result, error_content = ResponseChecker.check_polish_response_content(self, config, response_content,
    #                                                                                     response_dict, source_text_dict)
    #
    #         if not check_result:
    ...
    #
    #         # 发送表格更新信号
    #         self.event_emit(TransBase.EVENT.TABLE_UPDATE, {
    #             "file_path": file_path,
    ...
    #         TransBase.work_status = TransBase.STATUS.IDLE  # 任务失败，重置状态
    #         return
    #
    #     # 日志输出
    #     print("├─ 收到回复，内容如下:")
    ...
    #
    #     # 发送单次表格更新信号
    #     self.event_emit(TransBase.EVENT.TABLE_FORMAT, {
    #         "file_path": file_path,
    ...
    def process_term_extraction(self, data: dict):
        ...
        #     self.event_emit(TransBase.EVENT.TERM_EXTRACTION_DONE, {"results": []})
        #     return
        #
        ...
        # # self.event_emit(TransBase.EVENT.TERM_EXTRACTION_DONE, {"results": results})
        return
...
#             self.event_emit(TransBase.EVENT.TERM_TRANSLATE_SAVE_DONE, {"status": "no_result", "message": "未解析到有效术语"})
#             return
#
#         # 加载配置文件
...
#         self.event_emit(TransBase.EVENT.TERM_TRANSLATE_SAVE_DONE, {
#             "status": "success",
